...
*/
_xbee_atcmd_debug
		xbee_cmd_request_t FAR *
		_xbee_cmd_handle_to_address(int16_t handle)
{
	xbee_cmd_request_t FAR *request;
...
*/
_xbee_atcmd_debug int xbee_cmd_list_execute(
		xbee_dev_t *xbee,
		xbee_command_list_context_t FAR *clc,
...
*/
_xbee_atcmd_debug enum xbee_command_list_status(xbee_cmd_list_status)(
		xbee_command_list_context_t FAR *clc)
{
	return clc->status;
}
...
_xbee_atcmd_debug void _xbee_cmd_query_handle_eo(
		const xbee_cmd_response_t FAR *response,
...
_xbee_atcmd_debug void _xbee_cmd_query_handle_ai(
		const xbee_cmd_response_t FAR *response,
		const struct xbee_atcmd_reg_t FAR *reg,
		void FAR *base)
{
...
_xbee_atcmd_debug void _xbee_cmd_query_handle_end(
		const xbee_cmd_response_t FAR *response,
		const struct xbee_atcmd_reg_t FAR *reg,
		void FAR *base)
{
...
_xbee_atcmd_debug int xbee_cmd_query_device(xbee_dev_t *xbee, uint_fast8_t refresh)
{
	...
	if (error == 0)
	{
		...
		xbee->flags &=
				~(XBEE_DEV_FLAG_QUERY_ERROR | XBEE_DEV_FLAG_QUERY_DONE | XBEE_DEV_FLAG_QUERY_REFRESH);
	}
...
_xbee_atcmd_debug
		int16_t
		xbee_cmd_create(xbee_dev_t *xbee, const char FAR command[3])
{
...
*/
_xbee_atcmd_debug int xbee_cmd_set_callback(int16_t handle, xbee_cmd_callback_fn callback,
																						void FAR *context)
{
	xbee_cmd_request_t FAR *request;
...
_xbee_atcmd_debug int xbee_cmd_set_target(int16_t handle, const addr64 FAR *ieee,
...
_xbee_atcmd_debug
		uint8_t
		_xbee_cmd_encode_param(void FAR *buffer, uint32_t value)
{
...
/**
	...
	@see	xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_callback(),
			...
			xbee_cmd_send()
*/
_xbee_atcmd_debug int xbee_cmd_set_param_bytes(int16_t handle, const void FAR *data,
																							 uint8_t length)
{
	xbee_cmd_request_t FAR *request;
...
_xbee_atcmd_debug int xbee_cmd_send(int16_t handle)
{
	...
	if (request->flags & XBEE_CMD_FLAG_REMOTE)
	{
		...
		header.remote.options = (request->flags & XBEE_CMD_FLAG_QUEUE_CHANGE) ? XBEE_REMOTE_AT_OPT_QUEUE : XBEE_REMOTE_AT_OPT_IMMEDIATE;
...
#endif
	{
		header.frame_type = (request->flags & XBEE_CMD_FLAG_QUEUE_CHANGE) ? XBEE_FRAME_LOCAL_AT_CMD_Q : XBEE_FRAME_LOCAL_AT_CMD;
...
*/
_xbee_atcmd_debug int _xbee_cmd_handle_response(xbee_dev_t *xbee, const void FAR *rawframe,
...
/**
	...
	@see	xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_callback(),
			...
			xbee_cmd_execute()
*/
_xbee_atcmd_debug int xbee_cmd_simple(xbee_dev_t *xbee, const char FAR command[3],
																			uint32_t value)
{
	PACKED_STRUCT
	{
...
*/
_xbee_atcmd_debug int xbee_cmd_execute(xbee_dev_t *xbee, const char FAR command[3],
